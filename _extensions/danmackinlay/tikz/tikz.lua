...
-- Function to check if a command exists. Shell-builtin probes like `command -v`
-- (POSIX) or `where` (Windows) are brittle across platforms, so we instead try
-- to execute the tool with `--version` through pandoc.pipe: if the executable
-- is missing, pandoc.pipe raises and pcall returns false. (Local patch: the
-- upstream `command -v` form fails under Windows cmd.exe; this works on both
-- Windows and the Linux CI runner. Assumes the probed tools — pdflatex,
-- dvisvgm — support `--version`, which they do.)
local function check_dependency(cmd)
  local ok = pcall(function() return pandoc.pipe(cmd, { '--version' }, '') end)
  return ok
end
...
local function compile_tikz_to_svg(code, user_opts, conf, basename)  -- Added conf and basename parameters
  ...
  local function process_in_dir(dir)
    return with_working_directory(dir, function()
      ...
      -- Use absolute paths. On Windows the pandoc.pipe subprocess does not
      -- inherit the chdir done by with_working_directory, so relative file
      -- names (and the TeX engine's default output dir) would resolve against
      -- the original cwd and pdflatex would fail to find its .tex. (Local
      -- patch; harmless on the Linux CI runner.)
      local base_path = pandoc.path.join { dir, base_filename }
      local tikz_file = base_path .. ".tex"
      local pdf_file = base_path .. ".pdf"
      local svg_file = base_path .. ".svg"
      local dvi_file = base_path .. ".dvi"
...
% --- Preâmbulo padrão das figuras do Manual de Matemática (centralizado aqui).
% --- Driver dvisvgm só em modo DVI (saída HTML); em PDF (\pdfoutput=1) o pgf
% --- detecta o driver pdftex sozinho. Tem de vir antes de \usepackage{tikz}. ---
\ifdefined\pdfoutput\ifnum\pdfoutput=0 \def\pgfsysdriver{pgfsys-dvisvgm.def}\fi\fi
\usepackage{tikz}
\usepackage{pgfplots}
...
\definecolor{manualred}{HTML}{D7433B}
\definecolor{manualgreen}{HTML}{3FB618}
\definecolor{manualyellow}{HTML}{FF7518}
\definecolor{manualgray}{HTML}{6C757D}
% Estilos reutilizáveis.
\tikzset{
  eixo/.style={->,>=stealth,gray,thin},
  curva/.style={manualblue,very thick},
...
\end{document}
      ...
      local latex_args = { '-interaction=nonstopmode', '-output-directory=' .. dir }
      -- Ask for DVI only when we actually convert to SVG via dvisvgm. In PDF
      -- output mode we keep the engine's default PDF output (otherwise we'd
      -- produce a .dvi and then fail to read the expected .pdf). (Local patch.)
      if conf.svg_engine == 'dvisvgm' and conf.output_format ~= 'pdf' then
...
local function build_texinputs()
  -- Path separator: ':' on Unix, ';' on Windows. Detect Windows via Lua's
  -- own directory separator (package.config) rather than pandoc.system.os,
  -- which returns 'mingw32' (not 'windows') under the Haskell runtime on
  -- Windows — that mismatch picked ':' and split drive letters like `C:`,
  -- producing a garbage TEXINPUTS. (Local patch; correct on Linux CI too.)
